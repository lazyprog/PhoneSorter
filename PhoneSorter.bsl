...
Процедура ВыполнитьОбработкуФайла()
	...
	ТекстСообщения = СтрШаблон("Всего: %1, корректных: %2, исключено %3", СокрЛП(КолВоНомеровВИсходнике), СокрЛП(МассивНомеров.Количество()), СокрЛП(МассивИсключений.Количество()));
	Сообщить(ТекстСообщения);
